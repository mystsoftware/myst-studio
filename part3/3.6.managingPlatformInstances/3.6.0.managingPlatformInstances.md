...
Enter any notes as to why we are peforming the action and click `Execute`.
...
### Check for and Correct Configuration Drift
Configuration drift is caused by inconsistent configuration across different environments. Configuration drift occurs naturally in data center environments when changes due to manual ad hoc changes.

MyST's approach to managing incremental change, allows us to define, test, promote and track configuration chnages across all environments and thus prevent configuration drift from occuring.
...
#### Check for Drift
MyST's Check for Drift capability allows to check the configuration of the Platform Instance as defined by its Platform Blueprint / Model against the actual WebLogic domain and report on differences.

From within the Platform Instance Summary window, click on the `Actions` drop-down and select `Check for Drift`, MyST will initiate the .
...
Once the `Reset Drift` action has completed, click on `View` under `Execution log`. This will open a window showing the result of the Drift, as show below.
...
* Re-set a Platform Instance to a previous known state, for example re-provision the Platform Instance based on an earlier version of the Platform Blueprint.
...
When terminating a Platform Instance running on infrastrcuture of type AWS On-Demand, MyST will also provide you with the option of terminarting the AWS Instances
...
MyST allows us to re-provision any previosuly terminated Platform Instance. 
From the side menu navigate to`Modeling` > `Platform Models`, this will display a list of existing Platform Models. 

To reprovision a terminated Platform Instance, select the corresponding Platform Model, MyST will display the Platform Model and Instance summary. Click on the `Actions` menu and select `Provision`.
...
Specify whether a server restart is required to apply the patch, then enter any notes as to why we are peforming the action and click `Execute`.
