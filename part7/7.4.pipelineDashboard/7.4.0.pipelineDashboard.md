...
Any change to any of these components results in a new revision of that component. MyST keeps track of all revisions and which revision of each of these components has been promoted in too each stage.
...
MyST's default behaviour when deploying a release, is to only deploys the artifacts that have been changed since the last deployment, or in the event of an Application Model change then redeploy all the artifacts contained within that Application Model.
